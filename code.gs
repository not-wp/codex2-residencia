...
function shiftDateFromAnchorSkippingRest(baseDate, restSet, anchorDate) {
  ...
  const anchor = anchorDate instanceof Date && !isNaN(anchorDate)
    ...
    : null;
  if (!anchor) {
    return shiftDateForwardSkippingRest(shifted, restSet);
  }
  anchor.setHours(0, 0, 0, 0);
  if (shifted.getTime() < anchor.getTime()) {
    return shiftDateForwardSkippingRest(shifted, restSet);
  }
  ...
  let guard = 0;
  let changed = true;
  while (changed && guard < safety) {
    changed = false;
    for (let i = 0; i < orderedRestDays.length; i++) {
      const restDay = orderedRestDays[i];
      restDay.setHours(0, 0, 0, 0);
      if (restDay.getTime() >= anchor.getTime() && restDay.getTime() <= shifted.getTime()) {
        shifted.setDate(shifted.getDate() + 1);
        changed = true;
      }
    }
...
function apiMakeReviewToday(payload) {
  try {
    ...
    // Distribui a meta diária de questões de forma ponderada pela prioridade de cada alvo pendente.
    const pendingItems = dueItems.filter(item => !asBoolean(item.feito));
    const totalPriority = pendingItems.reduce((sum, item) => {
    ...
    pendingItems.forEach(item => {
      ...
      const rawQuestions = totalPriority > 0
        ? (dailyQuestions * weight)
        : (dailyQuestions / Math.max(1, pendingItems.length));
      const suggested = Math.max(0, Math.floor(rawQuestions));
      item.questoesSugeridas = suggested;
      remainingQuestions -= suggested;
      item.__fraction = rawQuestions - suggested;
    });
    if (pendingItems.length > 0 && remainingQuestions > 0) {
      pendingItems
        .slice()
...
function apiGetReviewCalendar(days) {
  try {
    ...
    for (let i = 0; i < days; i++) {
      ...
      calendar[dateStr] = { quantidade: 0, descanso: restSet.has(formatDateISO(date)) };
    }
    ...
    spaced.forEach(item => {
      ...
      const proxEfetiva = shiftDateForwardSkippingRest(proxRevisao, restSet);
