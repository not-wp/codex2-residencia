...
function apiDashboardTotals() {
  try {
    ...
    if (!sheet || sheet.getLastRow() <= 1) {
      return { ok: true, totalQuestoes: 0, totalAcertos: 0 };
    }

    const headers = sheet.getRange(1, 1, 1, sheet.getLastColumn()).getValues()[0];
    const totalIdx = headers.indexOf('total');
    const acertosIdx = headers.indexOf('acertos');
    if (totalIdx < 0 || acertosIdx < 0) {
      return { ok: false, error: 'Colunas total/acertos não encontradas em LOG.', totalQuestoes: 0, totalAcertos: 0 };
    }
    ...
    values.forEach(row => {
      if (!row) return;
      const questoes = Math.max(0, safeNumber(row[totalIdx]));
      const acertos = Math.max(0, Math.min(safeNumber(row[acertosIdx]), questoes));
